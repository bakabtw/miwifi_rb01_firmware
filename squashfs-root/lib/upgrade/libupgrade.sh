...
flash_section() {
	...
	Preloader)
		...
		;;
	ATF)
		segment_name="ATF.bin"
		mtd_dev=$(get_mtd_device ${sec})
		;;
	uboot)
		segment_name="uboot.bin"
		mtd_dev=$(get_mtd_device ${sec})
		;;
	crash)
		...
		;;
	firmware)
		segment_name="firmware_squashfs.bin"
		os_idx=$(nvram get flag_boot_rootfs)
		if [ ${os_idx:-0} -eq 0 ]; then
			mtd_dev=$(get_mtd_device firmware1)
		else
			mtd_dev=$(get_mtd_device firmware)
		fi
		;;
	*)
