...
if [ "$netmod" = "whc_cap" -o "$netmod" = "whc_re" -o "$netmod" = "lanapmode" -a "$cap_mode" = "ap" ]; then
	...
	if [ "$ch_5g" = "auto" ]; then
		uci set wireless.$iface_ap_5g.channel_block_list='52,56,60,64,100,104,108,112,116,120,124,128,132,136,140,149,153,157,161,165'
		uci set wireless.$iface_bh_5g.channel_block_list='52,56,60,64,100,104,108,112,116,120,124,128,132,136,140,149,153,157,161,165'
		uci commit wireless
