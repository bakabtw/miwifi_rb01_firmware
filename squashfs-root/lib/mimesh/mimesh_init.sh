...
__update_auto_channel()
{
	local device="$1"
	local band=$(uci -q get wireless.${device}.band)

	[ "$band" == "5G" ] && {
		uci -q set wireless.${device}.AutoChannelSkipList="149;153;157;161;165"
	}
}
...
__init_network_re()
{
	...
	# RE: need make sure hnat on for high throughput performance
...
__init_cap()
{
	...
	config_foreach __update_auto_channel wifi-device
	uci commit wireless
